...
namespace gps {
	...
	glm::vec3 computeLocation(std::vector<glm::vec3> vertexes) {
		glm::vec3 result = glm::vec3(0.0f, 0.0f, 0.0f);
		unsigned sz = vertexes.size();
	...
	void Model3D::ReadOBJ(std::string fileName, std::string basePath) {
		...
		std::vector<glm::vec3> allVertexes;
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				for (size_t v = 0; v < fv; v++) {
					...
					allVertexes.push_back(currentVertex.Position);
		...
		this->location = computeLocation(allVertexes);
	}
